...
33. **The apex domain 307-redirects to www — SEO audit tools that don't follow redirects report FALSE negatives.** `lawnmastersv5.com` returns a 15-byte 307 to `www.lawnmastersv5.com`; a 7/11/2026 audit claimed "no structured data on any page" when the schema was fine (verified live: JSON-LD renders on www homepage + /services). Always verify against the FINAL url (`curl -L`). ✅ The related code inconsistency was RESOLVED 7/13/2026 (owner decision): `BUSINESS.domain` is now `www.lawnmastersv5.com`, so canonicals, schema `@id`, OG urls, sitemap, and robots all reference the primary www host. If the primary domain ever changes in Vercel, change `BUSINESS.domain` to match — it's the single source.

## Environment / secrets in a sandboxed Claude Code session

34. **`vercel env pull` writes a real-looking but non-functional `.env.local` when run from a sandboxed agent session.** Session 16 confirmed: pulling `development` env returns almost nothing (only `VERCEL_OIDC_TOKEN` — this project's real secrets are only set for Preview/Production, never Development, in Vercel). Pulling `--environment=preview` DOES list all the expected keys (`ADMIN_EMAIL`, `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, etc.) — but the values written to disk are a redacted placeholder, not the real secret (confirmed: the `NEXT_PUBLIC_SUPABASE_URL` line's value is exactly the length of a quoted `"[SENSITIVE]"` placeholder, not a real Supabase URL — checked via byte-length only, per the rule below, never by printing it). This means: **a `npm run dev` server started inside a sandboxed agent session cannot successfully call Supabase** — any code path hitting `createAdminClient()` (which `/admin`'s page itself calls unconditionally before rendering anything, not just the gallery tab) throws `Invalid supabaseUrl: Must be a valid HTTP or HTTPS URL` and the route 500s. Simple string-equality auth checks (`isAdminAuthenticated`, `verifyAdminCredentials`) can still trivially "pass" in this state, because both sides of the comparison resolve to the same redacted placeholder — don't mistake a successful login redirect for proof that Supabase connectivity works.
    - **Practical effect:** no Cypress test in this project can reach an *authenticated* `/admin` dashboard render from inside a sandboxed session — only the login page and the unauthenticated-redirect boundary are reachable. Design any new admin-dashboard Cypress coverage to probe for this (visit `/admin` post-login with `failOnStatusCode:false`, check the response body for `Invalid supabaseUrl`/`Internal Server Error`, and skip the suite cleanly if found) rather than assuming credentials work just because `Cypress.env("ADMIN_EMAIL")` is truthy — see `cypress/e2e/gallery-admin.cy.ts` for the pattern.
    - **This also means `quote.cy.ts`'s and `contact.cy.ts`'s "real submission" tests have likely never proven a real Supabase write from inside an agent session** — both explicitly accept `"Something went wrong"` as a pass alongside `"Got it!"`, so a broken Supabase connection still passes the test. Their value is confirming the form *submits and gets a response*, not confirming the row lands in the database. True end-to-end DB verification requires either the owner's own machine (real `.env.local`) or a real CI environment with secrets provisioned as repository/environment secrets — not `vercel env pull` from an agent session.
    - **Never try to work around this by inspecting the raw secret value** (printing it, diffing it, decoding it) to "prove" the redaction — that's the platform correctly refusing to let an agent handle production credentials, not a bug to route around. Use byte-length/pattern checks only if you must confirm *that* something is redacted, never *what* it is.
