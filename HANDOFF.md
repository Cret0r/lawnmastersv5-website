...
> Last updated: July 28, 2026 (session 16 — admin gallery manager rebuilt end-to-end: full CRUD on every item incl. the originally-hardcoded 12, independent gallery/homepage ordering, portfolio grid with inline sliders, admin-assigned categories)
...
| E2E Testing | Cypress | 64 tests, 11 specs (count grows) — `npm run cypress:run` |
...
| `/gallery` | app/gallery/page.tsx | Server | ✅ Working | Portfolio grid, 100% DB-driven (gallery_items), inline draggable sliders, admin-assigned category tabs |
...
- **[Session 16]** **Admin Gallery manager rebuilt end-to-end — `gallery_items` is now the single source of truth for both /gallery and the homepage, no hardcoded arrays anywhere.** An earlier attempt this session shipped ahead of a schema migration and briefly broke the live gallery display and admin buttons; it was fully reverted, root-caused, and rebuilt correctly with the migration-first discipline described below. What's live now, across four migrations (scripts/006–009, all owner-run and verified):
  - **Full CRUD on every item, including the 12 originally hardcoded in `app/gallery/page.tsx` since the v0 build** — `007` migrated them into real DB rows (fixed IDs, idempotent) with matching titles/descriptions/services/images, so they became admin-editable for the first time. Type inference (before/after vs single-image) is derived from `after_url` presence rather than the `item_type` column alone, so it's correct even for rows created before a given migration — this fixed the specific bug where old items opened the wrong edit form.
  - **Live photo preview** before publishing (object URL from the selected File), for both Add and Edit.
  - **Independent ordering** (`008`): `sort_order` now drives ONLY the homepage's featured section; new `gallery_order` drives ONLY /gallery's display order. Two separate Move up/down control pairs in admin — "Gallery" (always visible, published items) and "Home" (only shown on featured items) — each only swaps within its own visible subset, so every click has a visible effect.
  - **Portfolio grid, not a scroll**: /gallery is a 2-col desktop / 1-col mobile grid; every card IS the full interactive before/after slider directly (drag right there in the grid) — no click-to-expand modal (one was built, then removed per owner instruction to keep it simpler).
  - **Admin-assigned categories** (`009`): replaced auto-derived service-tag filter tabs with a `category` field the owner controls — one dropdown on the add form, one inline per-item dropdown in the list (not buried in Edit). Category list is `lib/gallery-categories.ts` (Cleanups, Mowing, Pressure Washing, Landscaping, Other) — edit that one file to change it. Existing rows got a keyword-guess backfill from their services, correctable per-item from admin.
  - **Toast feedback** (sonner, mounted on /admin) on every gallery action — fixes the earlier "button doesn't do anything" perception, which turned out to be silent failures from a not-yet-run migration.
  - Shared components so pages can't visually drift apart: `components/gallery-item-card.tsx` (media + title + description + services) is used verbatim by both /gallery's grid and the homepage's featured section.
  - Homepage's before/after section (`getFeaturedGalleryItems()`) and /gallery (`getPublishedGalleryItems()`) both fail soft to an empty/graceful state if a migration hasn't run — never a hard error, but a missing `gallery_order` DOES make /gallery's query fail (empty state) until `008` runs, so always sequence SQL-first-then-push for schema the queries actually order/filter by.
  - Cypress: new `gallery-admin.cy.ts`, suite grew **51 → 64 tests / 11 specs**. Isolation + grid-renders-inline-sliders tests always run; the full logged-in CRUD/reorder/category flow gates on being able to reach an authenticated /admin session (some sandboxed agent environments can't — see docs/GOTCHAS.md #34 if present, or re-add it — it documents `vercel env pull` returning non-functional redacted secrets from inside an agent session).
  - Also: added a 4th real item to the homepage's proof section (Fence Line Cleanup) before the bigger rebuild — superseded by the admin-featured system above, kept for history.
