...
> Last updated: July 27, 2026 (session 16 — admin gallery manager fully rebuilt: edit, feature, publish, reorder, single-image items)
...
| E2E Testing | Cypress | 59 tests, 11 specs (count grows) — `npm run cypress:run` |
...
- **[Session 16]** **Admin Gallery manager fully rebuilt — owner asked for full day-to-day control without touching code.** (1) Add form: live thumbnail preview from a `File` object URL under each photo input before submitting (revoked on change/unmount), plus an item-type toggle for **single-image items** (equipment/branded/finished-job shots — one photo, no slider). (2) **Edit** dialog (Radix, matches clients-tab pattern): title/description/services + optional photo replace with the same preview, updates the row in place (no duplicate), deletes the old Storage object only if the replacement got a different file extension. (3) **Feature star**: homepage's "Real Before & After Transformations" section is now 100% admin-controlled (`getFeaturedGalleryItems()` in lib/gallery.ts) — **the old hardcoded 3-item array (Florida-era photos captioned Covington/Conyers) was deleted outright**, not just superseded; the section renders nothing until the owner features a real item (no fallback, by design — flagged to owner). (4) Publish/unpublish eye toggle (bonus — column existed since session 8, no UI until now; unpublishing auto-unfeatures) + Move ↑/↓ reordering (reuses `sort_order`, normalizes-then-swaps since most rows shared the default 0). (5) New migration `scripts/007_gallery_enhancements.sql` (**owner must run this** — adds `featured` bool + `item_type` + relaxes `after_url` to nullable; no new RLS policy needed, table stays service_role-only). (6) Security re-verified end-to-end: grepped for any admin import outside `app/admin/`, confirmed `createAdminClient()` never reaches a client bundle, added `cypress/e2e/gallery-admin.cy.ts` asserting zero admin controls render on /gallery or `/` for a logged-out visitor. (7) **Sandbox discovery (see GOTCHAS #34):** `vercel env pull` from inside this agent session cannot retrieve working Supabase secrets even with `--environment=preview` (values come back as a redacted placeholder) — `/admin`'s own page unconditionally calls `createAdminClient()` before rendering anything, so no Cypress test in this kind of session can reach an authenticated dashboard render. Built `gallery-admin.cy.ts` to probe for this and skip that half of the suite cleanly (4 pending) rather than fail; the public/admin-isolation tests (4) always run and passed. Full suite: **59 tests, 11 specs, 55 passing + 4 pending, 0 failing.** The owner (or a session with real local `.env.local`) should run the logged-in half for real once, per docs/sops/gallery-migration.md's manual walkthrough.
