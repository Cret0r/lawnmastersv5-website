...
export function GalleryTab() {
  ...
  const refresh = async () => {
    try {
      ...
      setLoadError("Could not load the gallery. Try refreshing the page.")
    } finally {
      setLoaded(true)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {!loaded ? (
          ...
          <Card>
            <CardContent className="py-8 text-center text-destructive text-sm">{loadError}</CardContent>
          </Card>
        ) : items.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-muted-foreground text-sm">
              No gallery items yet. Add one above — it appears on /gallery immediately and can be featured on the homepage.
            </CardContent>
          </Card>
        ) : (
            ...
            >
              <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <img
                    src={item.before_url}
                    alt={`${item.title} — ${getEffectiveItemType(item) === "single" ? "photo" : "before"}`}
                    loading="lazy"
